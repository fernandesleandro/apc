/**
 * Extrai links dos manuais portobsb e atualiza database.json (manual-proprietario).
 * Uso: node scripts/crawl-manual-links.js
 */
const fs = require('fs');
const path = require('path');

const DB_FILE = path.join(__dirname, '..', 'data', 'database.json');
const BASE_HOST = 'www.portobsb.com.br';
...
async function collectManualLinks(pageUrl, manualRoot) {
  ...
  for (const frameUrl of frameUrls) {
    try {
      ...
      console.warn(`    frame ${frameUrl}: ${error.message}`);
    }
  }
...
async function crawlDocument(doc) {
  ...
  if (!href || /\.pdf($|\?)/i.test(href)) {
    return { ...doc, type: doc.type || inferDocType(doc.title, href), children: [] };
  }
  ...
  try {
    ...
    return {
      ...doc,
      type: doc.type || inferDocType(doc.title, href),
      children
    };
  } catch (error) {
    console.warn(`  ✗ ${doc.title}: ${error.message}`);
    return { ...doc, type: doc.type || inferDocType(doc.title, href), children: [] };
  }
}
...
async function main() {
  ...
  page.content.intro = 'Encontre o empreendimento, abra o card e leia o capítulo desejado aqui no portal.';
  ...
  for (const section of page.content.sections) {
    ...
    for (const doc of section.documents || []) {
      ...
      const enriched = await crawlDocument(doc);
      console.log(`${(enriched.children || []).length} link(s)`);
  ...
  console.log('\nConcluído. database.json atualizado.');
}
